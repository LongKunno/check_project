...
class AIRule(BaseModel):
    prompt: str = Field(..., description="Hướng dẫn để AI có thể tự tìm lỗi trong code. Ví dụ: 'Tìm các biến có tên vô nghĩa như a, b, c, x, y'")
    reason: str = Field(..., description="Why it is blocked")
    pillar: str = Field(default="Maintainability", description="Which pillar it violates")
...
class CompiledRules(BaseModel):
    ...
    ai_rules: List[AIRule] = Field(default_factory=list, description="AI-only rules for Abstract logic detection")
...
"""
    ...
    async def auto_fix_stream(self, failed_json: str, test_case_code: str, error_message: str):
        prompt = f"""
