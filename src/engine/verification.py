...
from src.engine.scanners import (
    _build_flat_meta,
    BaseScanner,
    RegexScanner,
    PythonASTScanner,
            ...
            with open(file_path, "r", encoding="utf-8") as f:
                ...
                if file_path.endswith(".py"):
                    try:
                        ...
                        logger.debug(
                            f"Skipped AST parse for {file_path}: SyntaxError (file may have invalid Python syntax)"
                        )
...
class VerificationStep:
    ...
    def _apply_disabled_rules(self, merged, disabled_ids):
        ...
        if disabled_ids:
            merged["rules"] = [
                r for r in merged.get("rules", []) if r.get("id") not in disabled_ids
            ]
        ...
        for r in merged.get("rules", []):
            ...
            if r_id and r_id in custom_weights:
                try:
                    ...
                    logger.warning(
                        f"Invalid custom weight for rule {r_id}: {custom_weights[r_id]}"
                    )
                ...
                merged.setdefault("rules", []).append(
                    {
                        ...
                        "ast": {
                            ...
                            "targets": [
                                {"name": node_name, "reason": r.get("reason", "")}
                            ],
                        },
                        "ai": None,
                    }
    ...
    def run_verification(self):
        ...
        try:
            ...
            try:
                ...
                raise ValueError(
                    f"AI Audit Output Error: Expected JSON but got: '{stdout_lines[-1][:100]}...'. Error: {jde}"
                )
