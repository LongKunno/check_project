...
class AstContextExtractor:
    ...
    def index_project(self):
        ...
        logging.getLogger(__name__).info(
            "   🔍 [Indexer] Đang lập chỉ mục mã nguồn (AST Indexing) để hỗ trợ AI..."
        )
    ...
    def _index_file(self, file_path):
        try:
            ...
            logging.getLogger(__name__).warning(
                f"Error parsing AST for {file_path}: {e}"
            )
            return
            ...
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                ...
                if hasattr(node, "end_lineno") and node.end_lineno:
                    ...
                    last_body = node.body[-1]
                        ...
                        last_body,
                        "end_lineno",
                        getattr(last_body, "lineno", start + 15),
                    )
                else:
                    end = start + 15
        ...
        for path, start, end in self.symbol_map[symbol_name]:
            try:
                with open(path, "r", encoding="utf-8") as f:
                    ...
                    snippets.append(
                        f"--- SOURCE CODE FROM: {rel_path} (Lines {start}-{end}) ---\n```python\n{snippet}\n```"
                    )
            except Exception as e:
                logging.getLogger(__name__).warning(
                    f"Error reading block from {path}: {e}"
                )
