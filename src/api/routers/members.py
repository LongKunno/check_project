...
def _aggregate_members(all_member_data: list[dict]) -> dict:
    ...
    for entry in all_member_data:
        ...
        if email not in aggregated:
            aggregated[email] = {
                ...
                "total_loc": 0,
                "total_debt_mins": 0,
                "total_punishments": {p: 0.0 for p in WEIGHTS.keys()},
                "projects": [],
            }
        ...
        for pillar, val in entry.get("punishments", {}).items():
            if pillar in agg["total_punishments"]:
                agg["total_punishments"][pillar] += val
    ...
    for email, agg in aggregated.items():
        ...
        # Tính pillar scores từ tổng punishment gộp
        pillar_scores = {}
        for pillar in WEIGHTS.keys():
            total_punishment = agg["total_punishments"].get(pillar, 0.0)
            pillar_scores[pillar] = ScoringEngine.calculate_pillar_score(
                total_punishment, total_loc, pillar
            )
