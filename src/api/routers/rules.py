...
def _run_scanners_sync(content, compiled_json):
    ...
    for key, val in ast_cfg.items():
        if key == 'dangerous_functions': continue
    ...
    try:
        ...
        if violations:
            ...
            for str_idx, res in review_results.items():
                ...
                if idx < len(violations):
                    violations[idx]['is_false_positive'] = res.get('is_false_positive', False)
        ...
        if ai_rules:
            ...
            custom_rules_for_deep = {
                "natural_text": "Quy định riêng: " + json.dumps(ai_rules, ensure_ascii=False)
            }
            files_chunk = [{"path": "sandbox.py", "content": request.code_snippet}]
            # Gọi deep_audit_batch để AI tự lùng sục lỗi
            deep_violations = await ai_service.deep_audit_batch(files_chunk, custom_rules=custom_rules_for_deep)
            
            for dv in deep_violations:
                dv['is_false_positive'] = False
                dv['ai_explanation'] = "Phát hiện trực tiếp bằng AI Deep Audit (Bỏ qua máy quét tĩnh)."
...
async def auto_fix_rule(request: AutoFixRequest):
    ...
    async def log_generator():
        try:
            async for chunk in compiler.auto_fix_stream(request.failed_json, request.test_case_code, request.error_message):
                yield chunk
        except Exception as e:
            yield f"\\n\\n[LỖI]: {str(e)}"
