...
import { Settings, AlertTriangle, Zap, Trash2, ShieldAlert, Activity, ShieldCheck, Code2, ExternalLink, Server, Clock, Cpu, Info, FolderOpen } from 'lucide-react';
import { motion } from 'framer-motion';
import { useToast } from '../ui/Toast';

const InfoCard = ({ icon, label, value, iconClass = 'bg-slate-500/10 border-slate-500/20 text-slate-400', accent = '' }) => (
  <div className={`flex items-center gap-3 p-4 rounded-2xl bg-white/[0.03] border backdrop-blur-sm transition-all hover:bg-white/[0.06] ${accent || 'border-white/8'}`}>
...
const SettingsView = ({ selectedRepoId, cn }) => {
  ...
  useEffect(() => {
    ...
    fetch(`/api/rules?target=${encodeURIComponent(selectedRepoId)}`)
      ...
      .then(resp => {
        if (!resp?.data) return;
        const d = resp.data;
        const coreCount = d.default_rules ? Object.keys(d.default_rules).length : 0;
        const customRegex = d.compiled_json?.regex_rules?.length || 0;
        const customAst = d.compiled_json?.ast_rules?.dangerous_functions?.length || 0;
        const disabledCount = d.disabled_rules?.length || 0;
        setRulesInfo({ coreCount, customCount: customRegex + customAst, disabledCount });
      })
      .catch(() => {});
  }, [selectedRepoId]);
  ...
  const handleReset = async () => {
    ...
    setIsResetting(true);
    try {
      const res = await fetch(`/api/rules?target=${encodeURIComponent(selectedRepoId)}`, { method: 'DELETE' });
      if (res.ok) {
        toast.success('All custom rules have been deleted and weights reset to default.', 'Rules Reset');
      } else {
    ...
    <div className="w-full flex-1 p-6 lg:p-8 max-w-4xl mx-auto relative z-10">
      ...
      <div className="space-y-6">
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            <InfoCard icon={<ShieldCheck size={16} />} label="Core Rules" value={rulesInfo ? `${rulesInfo.coreCount} rules` : '—'} iconClass="bg-emerald-500/10 border-emerald-500/20 text-emerald-400" accent="border-emerald-500/25 shadow-[0_0_15px_-5px_rgba(16,185,129,0.15)]" />
            <InfoCard icon={<Zap size={16} />} label="Custom AI Rules" value={rulesInfo ? `${rulesInfo.customCount} rules` : '—'} iconClass="bg-violet-500/10 border-violet-500/20 text-violet-400" accent="border-violet-500/25 shadow-[0_0_15px_-5px_rgba(139,92,246,0.15)]" />
            <InfoCard icon={<AlertTriangle size={16} />} label="Disabled Rules" value={rulesInfo ? `${rulesInfo.disabledCount} rules` : '—'} iconClass="bg-amber-500/10 border-amber-500/20 text-amber-400" accent="border-amber-500/25 shadow-[0_0_15px_-5px_rgba(245,158,11,0.15)]" />
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
              ...
              >
                <div>
                  <div className="text-sm font-bold text-white group-hover:text-blue-400 transition-colors">{link.label}</div>
