            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div className="col-span-2 grid grid-cols-2 gap-3">
                {[
                  ...
                  { label: 'Debt', value: formatDebt(member.total_debt_mins), icon: <Clock size={14} className="text-amber-400" />, title: `${member.total_debt_mins} minutes` },
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-[11px] text-slate-500 font-medium">
                            {proj.loc?.toLocaleString()} LOC — Debt: {formatDebt(proj.debt_mins)}
                          </div>
                        </div>
                      </div>
...
function MemberRow({ member, rank, onClick }) {
  return (
    ...
    >
      ...
      <td className="px-4 py-4">
        <div className="flex items-center gap-3">
          <div
            ...
            style={{ background: getAvatarGradient(member.author_name || member.email) }}
          >
      ...
      {/* Debt */}
      <td className="px-4 py-4 text-right">
        <span className={`text-sm font-semibold ${getDebtClass(member.total_debt_mins)}`} title={`${member.total_debt_mins} minutes`}>
    ...
    <div className="w-full flex-1 p-6 lg:p-8 max-w-7xl mx-auto relative z-10">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-5 page-header-compact">
        ...
        {!isLoading && members.length > 0 && (
          ...
          >
            {[
              { label: 'Total members', value: members.length, icon: <Users size={16} className="text-cyan-400" />, accent: 'border-cyan-500/25 shadow-[0_0_15px_-5px_rgba(6,182,212,0.15)]', accentLine: 'kpi-accent-cyan' },
              ...
              { label: 'Top Performer', value: topPerformer?.author_name?.split(' ').pop() || '—', icon: <Trophy size={16} className="text-emerald-400" />, accent: 'border-emerald-500/25 shadow-[0_0_15px_-5px_rgba(16,185,129,0.15)]', accentLine: 'kpi-accent-emerald' },
              { label: 'Total LOC contributed', value: formatLoc(totalLoc), icon: <Code2 size={16} className="text-violet-400" />, accent: 'border-violet-500/25 shadow-[0_0_15px_-5px_rgba(139,92,246,0.15)]', accentLine: 'kpi-accent-violet' },
            ].map((s) => (
      ...
      {isLoading ? (
        ...
        >
          <div className="overflow-x-auto">
            <table className="w-full premium-table" style={{ '--table-accent': 'rgba(6, 182, 212, 0.5)' }}>
              <thead>
              ...
              <tbody>
                {sorted.slice((memPage - 1) * memPageSize, memPage * memPageSize).map((member, idx) => (
                  <MemberRow
                    key={member.email}
                    member={member}
