...
import { Activity, Code2, FolderOpen, AlertTriangle, Zap, Users } from 'lucide-react';
import { motion } from 'framer-motion';
import { getScoreColorClass } from '../../utils/chartHelpers';

const HeroCard = ({ data, reportView, selectedMember, chartCurrentViolations, topImprovements }) => {
  const score = reportView === 'project'
    ? data?.scores?.final
  ...
  const pillars = reportView === 'project'
  ...
  const kpis = [
    {
      label: 'Lines of Code',
      value: reportView === 'project'
        ? data?.metrics?.total_loc?.toLocaleString()
        : (data?.scores?.members?.[selectedMember]?.loc || 0).toLocaleString(),
      icon: <Code2 size={16} />,
      accent: '#3b82f6',
    ...
    {
      label: 'Features',
      value: reportView === 'project' ? Object.keys(data?.scores?.features || {}).length : '-',
    ...
    },
    {
      ...
      accentLine: chartCurrentViolations.length > 100 ? 'kpi-accent-orange' : 'kpi-accent-amber',
    },
    {
      ...
      sub: reportView === 'project' && topImprovements[0] ? `${topImprovements[0].score}/100` : undefined,
    },
  ];
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: '3rem', alignItems: 'stretch' }}>
        {/* Left: Score + Rating */}
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flexShrink: 0, minWidth: '180px', justifyContent: 'center' }}>
          <div
            className="metric-label"
            style={{
              fontSize: '0.8rem', color: '#cbd5e1', display: 'flex', alignItems: 'center',
              gap: '0.5rem', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '1rem',
            }}
          >
            {reportView === 'member'
          ...
          <div style={{ position: 'relative', width: 150, height: 150, flexShrink: 0 }}>
            <svg viewBox="0 0 100 100" width={150} height={150}>
              ...
              <motion.circle
                cx="50" cy="50" r="45" fill="none" stroke={ringColor}
            ...
            <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
              <motion.span
                ...
                style={{ fontSize: '2.5rem', fontWeight: 900, color: ringColor, lineHeight: 1, fontFamily: 'Outfit, sans-serif' }}
              >
                {score}
              </motion.span>
            ...
            <div style={{ marginTop: '0.75rem' }}>
              <span
                ...
                style={{
                  ...
                  borderRadius: '10px', color: '#f8fafc', fontWeight: 700,
                }}
              >
                {data.scores.rating}
              </span>
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', alignItems: 'baseline' }}>
                  <span style={{ fontSize: '0.8rem', fontWeight: 700, color: '#e2e8f0', textTransform: 'uppercase', letterSpacing: '0.06em' }}>
                    {pillar}
                  </span>
                  <span style={{ fontSize: '1.1rem', fontWeight: 900, color, fontFamily: 'Outfit, sans-serif' }}>
                    {pScore}<span style={{ fontSize: '0.7rem', opacity: 0.4, marginLeft: '2px' }}>/10</span>
                  </span>
                </div>
                <div style={{ height: '6px', background: 'rgba(255,255,255,0.06)', borderRadius: '10px', overflow: 'hidden' }}>
                  <motion.div
                    ...
                    transition={{ duration: 0.8, delay: 0.3 + idx * 0.08, ease: [0.4, 0, 0.2, 1] }}
                    style={{
                      height: '100%',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', paddingTop: '1.5rem', borderTop: '1px solid rgba(255,255,255,0.05)' }}>
        {kpis.map((kpi, i) => (
          <motion.div
            key={i}
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: kpi.accent }}>
              {kpi.icon}
            ...
            <div
              style={{
                fontSize: '1.35rem', fontWeight: 900, color: '#f8fafc', fontFamily: 'Outfit, sans-serif',
            ...
            {kpi.sub && (
              <div style={{ fontSize: '0.7rem', color: kpi.accent, fontWeight: 700 }}>{kpi.sub}</div>
            )}
          </motion.div>
        ))}
      </div>
