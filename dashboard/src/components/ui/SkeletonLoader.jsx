    ...
    <div className="bg-[#0f172a]/80 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
      ...
      <div className="flex gap-4 px-6 py-4 border-b border-white/5 bg-white/2">
        {Array.from({ length: cols }).map((_, i) => (
          <div
            key={i}
            className="skeleton skeleton-line flex-1"
            style={{
              width: `${60 + Math.random() * 40}%`,
              maxWidth: i === 0 ? 180 : 120,
            }}
          />
        ))}
      </div>
      ...
      {Array.from({ length: count }).map((_, i) => (
        ...
        >
          ...
          <div className="flex-1">
            ...
            <div
              className="skeleton skeleton-line"
              style={{ width: "40%", height: 18, marginBottom: 0 }}
            />
          </div>
        </motion.div>
      ))}
...
export function HeaderSkeleton() {
  return (
    <div className="mb-8">
      <div
        className="skeleton"
        style={{ width: 140, height: 28, borderRadius: 9999, marginBottom: 16 }}
      />
      <div
        className="skeleton"
        style={{ width: "45%", height: 40, borderRadius: 12, marginBottom: 12 }}
      />
      <div
        className="skeleton"
        style={{ width: "65%", height: 16, borderRadius: 8 }}
      />
    </div>
  );
}
...
export function ScoreRingSkeleton({ size = 120 }) {
  return (
    ...
    >
      ...
      <div
        className="absolute skeleton skeleton-circle"
