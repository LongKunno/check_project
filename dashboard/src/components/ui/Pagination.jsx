...
const Pagination = ({
  ...
  const btnBase = 'flex items-center justify-center w-8 h-8 rounded-lg text-xs font-bold transition-all duration-200 select-none';
    ...
    <div className="px-5 py-3 border-t border-white/[0.06] flex flex-col sm:flex-row items-center justify-between gap-3 bg-white/[0.02]">
      ...
      <div className="flex items-center gap-3">
        <span className="text-[11px] text-slate-500 font-medium">
          Showing <span className="text-slate-400 font-semibold">{startItem}–{endItem}</span> of{' '}
          <span className="text-slate-400 font-semibold">{totalItems}</span> {label}
        </span>
        ...
        {showPageSizeSelector && onPageSizeChange && (
          ...
          >
            {pageSizeOptions.map(s => (
              <option key={s} value={s} className="bg-slate-900 text-slate-300">{s} / page</option>
            ))}
          </select>
        )}
      </div>
