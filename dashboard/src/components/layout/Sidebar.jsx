...
import { ChevronLeft, ChevronRight, Shield, FolderOpen,
  ...
  Settings, BarChart3, Users, Menu, X, MonitorPlay, Globe
} from 'lucide-react';
...
export const Sidebar = ({
  ...
  const repoNavItems = [
    { path: '/audit', label: 'Audit Dashboard', icon: Activity,
    ...
    { path: '/history', label: 'Audit History', icon: FileSearch,
  ...
  const bottomNavItems = [
    { path: '/presentations', label: 'Presentations', icon: MonitorPlay,
      activeClass: 'bg-rose-500/10 text-rose-400 border border-rose-500/20 shadow-inner glow-pink',
      iconClass: 'text-rose-500/80' },
  ];

  const renderNavItems = (items) => items.map(({ path, label, icon: Icon, activeClass, iconClass }) => (
    <button
      key={path}
          ...
          <div className={cn(
            ...
            {!isSidebarCollapsed ? (
              <>
                ...
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest opacity-70">Global Views</span>
              </>
            ) : (
              <span className="text-[10px] font-bold text-slate-600 opacity-70">•••</span>
            )}
          </div>
        ...
        <div className={cn(
          ...
          {!isSidebarCollapsed ? (
            <>
              ...
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest opacity-70">Repository Workspace</span>
            </>
          ) : (
            <span className="text-[10px] font-bold text-slate-600 opacity-70">•••</span>
          )}
        ...
        <div className={cn(
          ...
          {!isSidebarCollapsed ? (
            <>
              ...
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest opacity-70">System Info</span>
            </>
          ) : (
            <span className="text-[10px] font-bold text-slate-600 opacity-70">•••</span>
          )}
