...
def test_authorship_tracker():
    ...
    # Commit 1 year ago
    subprocess.run(['git', 'commit', '-m', "Old commit", '--date="1 year ago"'], cwd=test_dir, check=True)
    ...
    print("Line 1:", info1)
    assert info1['boundary'] == True, "Dòng 1 phải nằm ngoài vùng 6 tháng (boundary=True)"
    
    # Truy vấn line 3 (commit hôm nay, boundary = False)
    info3 = tracker.get_author_info('main.py', 3)
    print("Line 3:", info3)
    assert info3['boundary'] == False, "Dòng 3 phải nằm trong vùng 6 tháng (boundary=False)"
    assert info3['author'] == "TestUser", "Dòng 3 phải thuộc về TestUser"

    # Kiểm tra LOC
    locs = tracker.get_all_member_loc()
    print("Membet LOC:", locs)
    assert locs.get("TestUser", 0) == 2, "TestUser phải có 2 dòng (line 3, 4)"
