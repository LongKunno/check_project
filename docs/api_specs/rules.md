...
Reset chọn lọc theo cấp độ (Multi-Level Reset).
- **Phương thức:** `POST`
- **Body:**
```json
{
  "target": "GLOBAL hoặc project_id",
  "level": "toggles | weights | custom | all"
}
```
- **Levels:**

| Level | Dữ liệu bị reset | Dữ liệu giữ lại |
|---|---|---|
| `toggles` | `disabled_core_rules`, `enabled_core_rules` | `custom_weights`, `compiled_json`, `natural_text` |
| `weights` | `custom_weights` | `disabled_core_rules`, `enabled_core_rules`, `compiled_json` |
| `custom` | `compiled_json`, `natural_text` | `disabled_core_rules`, `enabled_core_rules`, `custom_weights` |
| `all` | Xóa toàn bộ row (= DELETE) | Không giữ gì |

- **Phản hồi (200):**
```json
{
  "status": "success",
  "message": "Rule toggles (bật/tắt) đã được reset.",
  "level": "toggles",
  "target": "GLOBAL"
}
```
- **Phản hồi lỗi (400):** Level không hợp lệ.
- **Phản hồi lỗi (500):** Lỗi database.
