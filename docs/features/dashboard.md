...
7. **UI Overhaul V1 (2026-04-06)**:
 ...
 12. **Global Rules Reset (2026-04-15)**:
      - Thêm nút **Reset Global Rules to Defaults** trong `SettingsView.jsx` → Danger Zone.
      - Cho phép khôi phục toàn bộ cấu hình Global Rules (`disabled_core_rules`, `enabled_core_rules`, `custom_weights` của `target_id='GLOBAL'`) về trạng thái gốc.
      - **Danger Zone Redesign:** Tách thành 2 section rõ ràng trong Danger Zone:
        - **GLOBAL section** (badge xanh dương): Reset toàn bộ Global Rules — ảnh hưởng tất cả dự án.
        - **PROJECT section** (badge xanh lá): Reset rules của project đang chọn (hành vi cũ, giữ nguyên).
      - **Double-confirmation:** Mỗi nút có cơ chế xác nhận riêng biệt (click 2 lần) với timeout tự hủy (Global: 4s, Project: 3s).
      - **API:** Gọi `DELETE /api/rules?target=GLOBAL` — backend xóa row `GLOBAL` khỏi bảng `project_rules`.
