...
7. **UI Overhaul V1 (2026-04-06)**:
 ...
 12. **Multi-Level Reset System (2026-04-15)**:
      ...
      - **GLOBAL** (badge xanh dương):
        ...
        - 🟠 **Reset Penalties**: Reset `custom_weights` — đưa điểm phạt về mặc định
      ...
      - **PROJECT** (badge xanh lá):
        ...
        - 🟠 **Reset Penalties**: Reset `custom_weights` chỉ cho project
 ...
 16. **AI Cache Console (2026-04-22)**:
      - **Trang riêng** tại route `/ai-cache`, hiển thị trong Sidebar dưới nhóm global tools.
      - **Mục tiêu**: Cho phép vận hành AI cache mà không cần vào DB thủ công.
      - **Chức năng**:
        - Xem runtime policy: `enabled`, `validation_enabled`, `deep_audit_enabled`, `cross_check_enabled`, `retention_days`
        - Xem snapshot hiệu quả cache: số entry, last hit, hit rate, saved tokens, saved cost
        - Cập nhật policy qua `GET/PUT/DELETE /api/ai/cache`
        - Clear toàn bộ cache với xác nhận rõ ràng trước khi xoá
      - **Lưu ý vận hành**: Xoá cache không ảnh hưởng audit history đã lưu, nhưng các request AI kế tiếp sẽ phải tính lại từ đầu.

 17. **Baseline + Regression Gate (2026-04-23)**:
      - **Baseline cố định**: Mỗi lần scan sẽ so sánh với **scan liền trước** của cùng repository.
      - **Gate mềm**: Không fail audit job hay batch scan; chỉ gắn nhãn `warning` để người dùng nhận biết regression.
      - **Multi-signal**: Có thể kích hoạt cảnh báo từ nhiều tín hiệu cùng lúc:
        - Score tổng giảm vượt ngưỡng
        - Số finding tăng vượt ngưỡng
        - Bất kỳ pillar score nào giảm vượt ngưỡng
        - Có finding severity cao mới xuất hiện
      - **UI tích hợp**:
        - `ProjectScoresView.jsx`: thêm KPI `Regressing repos` và cột `Regression Gate`
        - `HistoryView.jsx`: hiển thị trạng thái gate cho từng audit row
        - `SettingsView.jsx`: cho phép chỉnh ngưỡng runtime ngay trên màn hình Settings
      - **Persistence**:
        - `audit_history` lưu thêm `baseline_audit_id`, `regression_status`, `regression_summary`
        - `full_json.metadata.regression` được enrich để audit detail có thể dùng lại cùng metadata

 18. **Trend Dashboard (2026-04-23)**:
      - **Trang riêng** tại route `/trends`, thay thế view review-detail cũ trong sidebar.
      - **Portfolio scope**:
        - KPI: scanned repos, avg latest score, regressing repos, scans in range
        - Biểu đồ: average score theo ngày, scan volume vs warning count
        - Snapshot: latest portfolio pillars, top regressing repositories
      - **Repository scope**:
        - Score trend, violations trend, pillar trend
        - Danh sách `regression_events` để xem nhanh các đợt xấu đi gần đây
      - **API**:
        - `GET /api/trends/portfolio?days=7|30|90`
        - `GET /api/trends/repository?target=<repo_url>&days=7|30|90`
      - **Lưu ý UX**: Nếu chưa có dữ liệu hoặc chưa chọn repo, UI render empty state thay vì lỗi.
