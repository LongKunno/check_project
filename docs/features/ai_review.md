...
- **Batch Size & Concurrency Optimization**: Kích thước batch khi xác thực vi phạm được giữ ở mức `5` lỗi/request để đảm bảo AI phân tích chính xác. Tuy nhiên, để tối ưu hóa tài nguyên và tốc độ, toàn bộ quá trình giao tiếp với AI API hiện sử dụng kiến trúc Bất đồng bộ (Asynchronous I/O) với `asyncio` và `AsyncOpenAI`, giúp thực hiện hàng chục request đồng thời siêu kiện và vượt qua rào cản blocking của ThreadPoolExecutor cũ.
