...
5. **Quản trị rủi ro & Điểm phạt (Penalty Override) trực quan:**
   - Cho phép **rê chuột dồn cuộn (Scroll/Wheel)** lên ô số để tinh chỉnh Điểm phạt của từng vi phạm tăng/giảm siêu mượt mà.
...
1. Mở trang UI. Nhấp Tab `Danh sách Rule` để sửa đổi Core Rules và Custom Rules bằng thanh trượt điểm phạt (Mouse wheel).
...
- **Tầng 1: Global Rules (Mặc định Hệ thống):** Toàn bộ nền tảng áp dụng chung một bộ luật (core rules). Quản trị viên (Global Admin) có quyền định dạng các thuộc tính như Trạng Thái (Bật/Tắt) hoặc Điểm phạt (`weight`) cho mục tiêu `target_id='GLOBAL'`.
...
- `custom_weights` (JSON Object): Các điểm phạt bị ghi đè riêng của Project.
...
- Badge **✦ Custom Penalty** ẩn khi `isGlobalCustomized=true` để tránh trùng lặp.
