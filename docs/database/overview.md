...
> [!IMPORTANT]
> Nếu thiếu `DATABASE_URL` hoặc Postgres chưa reachable, hệ thống **không giả lập audit history**. Chỉ riêng repository management được fallback sang danh sách in-memory từ `config.py`; các dữ liệu như audit history, change review history, rules override và engine settings sẽ không được persist.

## Các cấu trúc bảng (Schema)

Hệ thống hiện tại có các bảng cốt lõi cho repository, audit, rules và settings; ngoài ra còn có các bảng vận hành cho jobs và AI cache. Dưới đây là sơ đồ quan hệ của nhóm bảng lõi:
...
>
> Nếu DB không sẵn sàng, API repository vẫn đọc/ghi trên snapshot in-memory từ `config.py`. Chế độ này chỉ phù hợp cho local/dev vì thay đổi sẽ mất sau restart.
...
- `baseline_audit_id` (INTEGER nullable) - Audit liền trước dùng làm baseline.
- `regression_status` (TEXT) - `pass`, `warning`, `unavailable`.
- `regression_summary` (TEXT JSON) - Delta score/violations/pillars và danh sách tín hiệu đã trigger.
...
| `auth_required` | bool string | `"true"` (.env) | Bật/tắt bắt buộc đăng nhập |
| `regression_gate_enabled` | bool string | `"true"` (.env) | Bật/tắt soft warning regression |
| `regression_score_drop_threshold` | float string | `"2.0"` (.env) | Ngưỡng score drop để cảnh báo |
| `regression_violations_increase_threshold` | int string | `"5"` (.env) | Ngưỡng finding increase để cảnh báo |
| `regression_pillar_drop_threshold` | float string | `"0.5"` (.env) | Ngưỡng pillar drop để cảnh báo |
| `regression_new_critical_threshold` | int string | `"1"` (.env) | Ngưỡng finding severity cao mới |

> [!NOTE]
> Ưu tiên đọc: DB → .env (fallback). Helper functions: `get_ai_enabled()`, `get_ai_max_concurrency()`, `get_test_mode_limit()` trong `config.py`.

## Các bảng vận hành bổ sung

- `runtime_jobs`, `runtime_job_logs`: lưu trạng thái background jobs và SSE logs có thể khôi phục.
- `ai_cache_entries`, `ai_cache_runs`: lưu cache theo stage và thống kê tiết kiệm token/cost cho AI cache console.
...
> [!NOTE]
> Khi DB không sẵn sàng, `GET/POST/PUT/DELETE /api/repositories` vẫn hoạt động trên in-memory fallback. `GET /api/repositories/scores` vẫn trả danh sách repos, nhưng các trường audit-derived như `latest_score` hoặc `pillar_scores` sẽ là `null` nếu không có dữ liệu persist.
...
| PUT | `/api/settings/engine` | Cập nhật cấu hình engine (lưu DB, runtime reload; trả `503` nếu persistence unavailable) |
| GET | `/api/trends/portfolio` | Tổng hợp xu hướng score/warning toàn portfolio |
| GET | `/api/trends/repository` | Timeline score/violations/pillars của một repository |
