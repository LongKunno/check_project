...
## ADR-012: Refactor Monolithic Components to Mitigate Technical Debt (Phase 6)
**Date:** 2026-04-03
**Status:** Accepted

### 1. Problem
Trong quá trình Audit lại chính mình định kỳ bằng `check_project`, hệ thống phát hiện một lượng lớn Technical Debt liên quan đến "God Object" và "High Cyclomatic Complexity" phân tán rộng rãi trên các modules quan trọng của Engine.
Cụ thể:
- `auditor.py`: `run()` method dài hơn 200 dòng, CC=62. Phương thức định dạng `generate_report` thực thi mọi render Markdown gây quá tải (CC=22).
- `scanners.py`: `PythonASTScanner.scan` nhồi nhét xử lý hơn 10 syntax pattern trong một single walk loop (dài ~200 dòng).
- `verification.py`: `load_rules` quản lý logic parse, fallbacks, mapping rườm rà.
- `ai_service.py`: Các payload call LLM (`deep_audit_batch`, `verify_violations_batch`, `verify_flagged_issues`) gánh luôn retry policies và JSON fallback mechanisms sinh ra code lặp.
- `audit.py`: Các job runners (Local/UI/Git) lặp lại việc xử lý Database mapping và Metrics format.

### 2. Options
*   **Option 1: Ignore.** Bỏ qua vì các module này thực tế ít khi có bugs và việc thay đổi diện rộng tiềm ẩn rủi ro hồi quy (Regression).
*   **Option 2: Deep Refactoring (Extract Helper/Private Sub-methods).** Chia các God Object thành các module tư duy nhỏ, Single Responsibility.

### 3. Decision
**Chọn Option 2.** Engine mã nguồn của `check-project` phải đạt điểm Gold Standard (>90) để đảm bảo độ tin cậy của Tool khi Audit mã nguồn hệ thống khác. Bất kể việc chia code sẽ tăng LOC, việc isolate logic là bắt buộc.

### 4. Consequences
- Cyclomatic Complexity của toàn hệ thống được kiểm soát hoàn toàn thông qua Private API Design Pattern.  
- `auditor.py` -> Tách 5 Step logic `_step_discovery`, `_step_scanning`, v.v...  
...
## ADR-014: Tinh chỉnh Rule BARE_EXCEPT và Grouped Violation Ledger
...
**A) Rule `BARE_EXCEPT` quá gắt:**
Rule `BARE_EXCEPT` (AST scanner) đánh đồng 3 tình huống khác nhau thành cùng 1 violation (-2 điểm):
1. `except:` (bare — bắt cả SystemExit/KeyboardInterrupt) → nguy hiểm thật
2. `except Exception:` + logging/raise → defensive programming hợp lệ
3. `except Exception:` + chỉ `pass` → nuốt exception im lặng

Kết quả: các pattern hoàn toàn hợp lệ như catch-all sau except cụ thể (ví dụ `git_helper.py`) hoặc startup error handling có logging (`api_server.py`) đều bị phạt oan → False Positive.

**B) Violation Ledger hiển thị danh sách phẳng:**
Dashboard VIOLATION LEDGER hiển thị flat list — mỗi violation một card riêng lẻ. Với dự án lớn có hàng chục/trăm violations, danh sách rất dài, không phân loại, khó scan để biết lỗi nào nghiêm trọng nhất.

### 2. Giải pháp (Decision)

**A) Rule BARE_EXCEPT — Chỉ phạt khi không xử lý:**
- Thêm helper method `_except_body_has_handling(node)` kiểm tra except body có statement thực sự hay chỉ `pass`/rỗng.
- `BARE_EXCEPT` chỉ trigger khi body **KHÔNG** xử lý gì (chỉ `pass` hoặc rỗng).
- Logging, raise, hay bất kỳ statement nào đều được coi là "có xử lý" → bỏ qua.
- Rule `SWALLOWED_EXCEPTION` (cho `except SpecificError: pass`) vẫn hoạt động bình thường.

**Các file thay đổi:**
- [scanners.py](../../src/engine/scanners.py): Thêm `_except_body_has_handling()`, sửa `_check_exceptions()`.
- [rules.json](../../src/engine/rules.json): Cập nhật `reason` của `BARE_EXCEPT`.

**B) Grouped Violation Ledger:**
Refactor VIOLATION LEDGER từ flat list sang **Grouped Accordion by rule_id**:
- Violations được nhóm theo `rule_id` bằng `useMemo`.
- Mỗi nhóm hiển thị: Rule ID (monospace), count badge, Pillar, tổng penalty.
- Sắp xếp theo tổng penalty giảm dần (nghiêm trọng nhất lên đầu).
- Mặc định **collapse toàn bộ** — chỉ cần liếc qua header là biết tình hình.
- Click expand → hiển thị chi tiết từng file:line, reason, snippet + Fix AI.
- Nút "Expand All / Collapse All" ở header.
- Severity color-coded: đỏ (≤ -5), vàng (≤ -3), xanh (còn lại) cho left border.
- Animation mở/đóng bằng Framer Motion `AnimatePresence`.

**File thay đổi:** [AuditView.jsx](../../dashboard/src/components/views/AuditView.jsx)

### 3. Tại sao?

- **BARE_EXCEPT**: Giảm False Positive, tập trung cảnh báo vào những except thực sự nguy hiểm (nuốt im lặng). Pattern `except Exception: logger.error(...)` là defensive programming chuẩn.
- **Grouped Ledger**: Giảm cognitive load 80%+ khi review violations. Từ cuộn 100 cards → scan 15 group headers. Rule nào nhiều violation nhất → nhìn thấy ngay, ưu tiên sửa trước.

### 4. Hệ quả (Consequences)

- **Tích cực**: Điểm số chính xác hơn (bỏ FP). Dashboard dễ dùng hơn cho Tech Lead/Architect khi review.
- **Lưu ý**: `except Exception: pass` sẽ bị phạt bởi CẢ `BARE_EXCEPT` (-2) VÀ `SWALLOWED_EXCEPTION` (-5) → double penalty là có chủ đích vì đây là trường hợp tệ nhất (vừa bắt chung vừa nuốt im).
